...
function initData($wrapper,$table,counterManage) {
    var _table = $table.dataTable($.extend(true,{},CONSTANT.DATA_TABLES.DEFAULT_OPTION, {
        ...
        "createdRow": function ( row, data, index ) {
            ...
            // $('td', row).eq(11).append($btnEdit).append($btnDel);
...
var counterManage = {
        ...
        if (data.order&&data.order.length&&data.order[0]) {
            switch (data.order[0].column) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    param.orderColumn = "counterStatus";
                    break;
                case 5:
                    param.orderColumn = "creater";
                    break;
                case 6:
                    param.orderColumn = "createDate";
                    break;
                case 7:
                    param.orderColumn = "lastModifyName";
                    break;
                case 8:
                    param.orderColumn = "lastModifyDate";
                    break;
                default:
    ...
    addItemSubmit : function() {
        ...
        param.counterName = $("#counterName").val();
        param.whseCode = $("#whseCode_add").val();
        param.counterStatus = $("#counterStatus").val();
        param.description = $("#desc").val();
        //如若柜台编码和仓库编码为空，则提示必填
        if(param.counterCode == "" || $.trim(param.counterCode).length == 0 || param.whseCode== "" || $.trim(param.whseCode).length == 0){
            alert("*内容不能为空！");
        }else{
            //仓库的柜台编码不能重复
            $.ajax({
                    url:"/counter/isExist",
                    data:{"whseCode":param.whseCode,"counterCode":param.counterCode},
                    type:"POST",
